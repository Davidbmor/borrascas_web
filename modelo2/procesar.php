...
function resolver_expediente_m2(array $registro, string $cifNif): array {
    $año = date('Y');
    ...
    if ($expedienteBase !== null) {
        ...
        $expedienteBase = 'Md2_' . $año . '_' . str_pad((string)$seq, 5, '0', STR_PAD_LEFT);
        return [
            'expediente_base'     => $expedienteBase,
            'revision'            => 0,
...
$rdtoValUser         = isset($_POST['rendimiento_valor']) && $_POST['rendimiento_valor'] !== '' ? abs((float)$_POST['rendimiento_valor']) : null;
$rdtoTipoUser        = trim((string)($_POST['rendimiento_tipo'] ?? 'pct'));
$prodEstimadaKg      = abs((float)($_POST['prod_estimada_kg']      ?? 0));
$prodRealKg          = abs((float)($_POST['prod_real_m2_kg']       ?? 0));
$aceitunaSueloVal     = abs((float)($_POST['aceituna_suelo_valor'] ?? 0));
$aceitunaSueloTipo    = trim((string)($_POST['aceituna_suelo_tipo'] ?? 'kg'));
...
$expInfo = resolver_expediente_m2($registro, $cifNif);
...
<ul style="margin:5px 0 10px 18px;padding:0;font-size:9.5px;">
  ...
  <li>Disminuci&oacute;n de reservas de carbohidratos;</li>
  <li>Reducci&oacute;n de la inducci&oacute;n floral;</li>
  <li>Incremento de enfermedades radiculares (<em>Phytophthora spp.</em>, <em>Armillaria spp.</em>);</li>
  <li>Dificultades para realizar abonados y tratamientos en los momentos &oacute;ptimos.</li>
</ul>
Todo ello puede traducirse en una disminuci&oacute;n significativa de la cosecha siguiente.<br><br>
El % reducci&oacute;n fisiol&oacute;gica por el encharcamiento y exceso de humedad se calcula en base al nivel de afecci&oacute;n:
<table class="datos" style="margin-top:6px;margin-bottom:8px;">
  <thead>
    <tr>
      <th>Nivel de afecci&oacute;n</th>
      <th>Situaci&oacute;n observada</th>
      <th>Reducci&oacute;n fisiol&oacute;gica estimada</th>
    </tr>
  </thead>
  <tbody>
    <tr><td><strong>Baja</strong></td><td>Encharcamiento &lt;10 d&iacute;as. Se pudieron realizar la mayor&iacute;a de labores. Sin s&iacute;ntomas radiculares.</td><td>5 &ndash; 10 %</td></tr>
    <tr><td><strong>Moderada</strong></td><td>Encharcamientos repetidos. Retraso en abonado y tratamientos. Ligera p&eacute;rdida de hojas o vigor.</td><td>10 &ndash; 20 %</td></tr>
    <tr><td><strong>Alta</strong></td><td>Encharcamientos &gt;20-30 d&iacute;as. Asfixia radicular. Tratamientos imposibles durante semanas. Defoliaci&oacute;n parcial.</td><td>20 &ndash; 35 %</td></tr>
    <tr><td><strong>Muy alta</strong></td><td>Inundaci&oacute;n prolongada. Da&ntilde;os radiculares importantes. Aparici&oacute;n de Phytophthora, erosi&oacute;n y p&eacute;rdida de suelo.</td><td>35 &ndash; 50 %</td></tr>
  </tbody>
</table>
...
<table class="datos" style="margin-top:6px;margin-bottom:10px;">
  ...
  <tbody>
    <tr><td><strong>Olivar</strong></td><td>P&eacute;rdida de producci&oacute;n</td><td style="text-align:right;">4,18 &euro;/kg. aceite <sup>1</sup></td></tr>
    <tr><td><strong>Olivar</strong></td><td>P&eacute;rdida de producci&oacute;n pr&oacute;xima cosecha</td><td style="text-align:right;">4,18 &euro;/kg. aceite</td></tr>
    <tr><td><strong>Olivar</strong></td><td>P&eacute;rdida calidad</td><td style="text-align:right;">1,50 &euro;/kg. aceite <sup>2</sup></td></tr>
    <tr><td><strong>Olivar</strong></td><td>Incremento costes de recolecci&oacute;n</td><td style="text-align:right;">0,25 &euro;/kg. aceite</td></tr>
...
<ul style="font-size:8.5px;color:#444;line-height:1.4;padding-left:15px;margin-top:4px;">
  ...
  <li>Fern&aacute;ndez, J.E. (2014). <em>Understanding olive adaptation to abiotic stresses as a tool to increase crop performance</em>. Environmental and Experimental Botany, 103, 158&ndash;179.</li>
  ...
  <li>Informaci&oacute;n sectorial procedente de cooperativas agrarias implantadas en las comarcas objeto del estudio.</li>
  <li>Informe Meteorol&oacute;gico del mes de febrero de 2026 de la Red de Alerta e Informaci&oacute;n Fitosanitaria (RAIF).</li>
  <li>Penco Valenzuela, J.M. (2024). <em>Aproximaci&oacute;n a los costes del cultivo del olivo</em>. Estudio AEMO, V5.</li>
  <li>Kozlowski, T.T. (1984). <em>Plant responses to flooding</em>. BioScience, 34(3), 162&ndash;167.</li>
  <li>Lorite I.J., Alza J., Cabezas J.M. (2026). <em>An&aacute;lisis de los eventos de lluvias extremas ocurridos en Andaluc&iacute;a en 2026</em>. Instituto de Investigaci&oacute;n y Formaci&oacute;n Agraria y Pesquera, 2026. 1-19 p. Consejer&iacute;a de Agricultura, Pesca, Agua y Desarrollo Rural.</li>
</ul>';
$textoDescDanos = 'La explotaci&oacute;n presents da&ntilde;os ocasionados por el temporal de lluvias y vientos:<br>
<table class="datos" style="margin-top:6px;">
  <tr><th>Da&ntilde;os Agron&oacute;micos</th><td>Saturaci&oacute;n h&iacute;drica del suelo, encharcamientos prolongados, ca&iacute;da de fruto al suelo, necrosis en fruto, aparici&oacute;n de plagas/enfermedades, rotura de ramas, &aacute;rboles arrancados y defoliaci&oacute;n.</td></tr>
  <tr><th>Da&ntilde;os Estructurales</th><td>Deterioro de accesos, erosi&oacute;n y c&aacute;rcavas, da&ntilde;os en infraestructura de riego (cabezal, balsa, tuber&iacute;as, acequias), almacenes agr&iacute;colas y vallado perimetral.</td></tr>
</table>';

// ─── CÁLCULOS OLIVAR (Apartado 7) ─────────────────────────────────
// Rendimiento Industrial (%) especificado o por defecto (20%)
if ($rdtoValUser !== null && $rdtoValUser > 0) {
    if ($rdtoTipoUser === 'kg_aceite') {
        // Se indicaron Kg de Aceite directamente -> Rendimiento = Kg aceite / Kg aceituna real
        $rendimiento = ($prodRealKg > 0) ? ($rdtoValUser / $prodRealKg) : 0.20;
    } else {
        // Se indicó Porcentaje % (máx 100%)
        $rendimiento = min(1.0, $rdtoValUser / 100);
    }
} else {
    $rendimiento = 0.20; // 20% por defecto
}

$precioAceite   = 4.18; // €/kg aceite
$depreciacion   = 1.50; // €/kg aceite (Diferencia AOVE y Lampante)
$costeAlmazara  = 0.04; // €/kg aceituna suelo

// Producciones a Aceite Campaña 2025/2026
$prodEstimadaAceite = $prodEstimadaKg * $rendimiento;
$prodRealAceite     = $prodRealKg * $rendimiento;

// Aceituna de Suelo (en Kilos)
if ($aceitunaSueloTipo === 'pct') {
    $pct = min(1.0, $aceitunaSueloVal / 100);
    $kgSueloAceituna = $prodRealKg * $pct;
} else {
    $kgSueloAceituna = $aceitunaSueloVal;
}
$kgSueloAceite = $kgSueloAceituna * $rendimiento;

// 1. Pérdida de producción campaña afectada (2025/2026)
$perdidaProdKgAceite = max(0, $prodEstimadaAceite - $prodRealAceite);
$valorPerdidaProd    = $perdidaProdKgAceite * $precioAceite;

// 2. Pérdida campaña siguiente (2026/2027) - Daño diferido
...
if ($nivelAfeccion === 'Muy alta') $reduccionFisiologica = ($drenajeParcelas === 'Bueno') ? 0.35 : 0.50;

// Producción prevista para la próxima campaña (2026/2027)
$baseProxKg = ($prodPrevistaProxKg > 0) ? $prodPrevistaProxKg : $prodEstimadaKg;
$baseProxAceite = $baseProxKg * $rendimiento;
$valorPerdidaProxima = $baseProxAceite * $reduccionFisiologica * $precioAceite;

// 3. Depreciación de calidad (Lampante)
$valorDepreciacion = $kgSueloAceite * $depreciacion;

// 4. Sobrecostes Recolección y Almazara
$costeRecoleccionRef = match($sistCultivo) {
    'OTNM'  => 0.33,
    'OTM'   => 0.20,
    'OI'    => 0.15,
    'OS'    => 0.08,
...
$pctSobrecoste = 0.30; // 30% promedio
$valorSobrecosteRec = $kgSueloAceituna * $costeRecoleccionRef * $pctSobrecoste;
$valorSobrecosteAlm = $kgSueloAceituna * $costeAlmazara;
$valorSobrecostes   = $valorSobrecosteRec + $valorSobrecosteAlm;
...
$textoValoracion = 'Para la elaboraci&oacute;n de la valoraci&oacute;n econ&oacute;mica de la explotaci&oacute;n de olivar se han utilizado los datos de producci&oacute;n declarados, el rendimiento industrial de la explotaci&oacute;n (' . number_format($rendimiento * 100, 2, ',', '.') . '%) y los par&aacute;metros del sector ole&iacute;cola:<br>

<table class="datos" style="margin-top:6px;margin-bottom:12px;">
  <tr><th style="width:40%;">Rendimiento industrial aplicado</th><td><strong>' . number_format($rendimiento * 100, 2, ',', '.') . '%</strong> ' . ($rdtoValUser !== null ? '(&Decl;arado)' : '(Estimaci&oacute;n de referencia 20%)') . '</td></tr>
  <tr><th>Campa&ntilde;a 2025/2026: Estimada vs Real</th><td>' . ($prodEstimadaKg > 0 ? number_format($prodEstimadaKg, 0, ',', '.') . ' Kg ac.' : 'Estimaci&oacute;n t&eacute;cnica') . ' &rarr; ' . ($prodRealKg > 0 ? number_format($prodRealKg, 0, ',', '.') . ' Kg ac. recolectados' : 'Pendiente') . '</td></tr>
  <tr><th>Aceituna real entregada de suelo</th><td>' . ($kgSueloAceituna > 0 ? number_format($kgSueloAceituna, 0, ',', '.') . ' Kg aceituna (' . number_format($kgSueloAceite, 2, ',', '.') . ' Kg aceite)' : '0 Kg') . '</td></tr>
  <tr><th>Campa&ntilde;a 2026/2027: Previsi&oacute;n sin borrascas</th><td>' . number_format($baseProxKg, 0, ',', '.') . ' Kg aceituna (' . number_format($baseProxAceite, 2, ',', '.') . ' Kg aceite)</td></tr>
...
<table class="datos" style="margin-top:6px;width:100%;border-collapse:collapse;font-size:8.5px;">
  <thead>
    <tr style="background:#2d6a4f;color:#fff;">
      ...
      <th style="text-align:right;padding:5px;width:18%;color:#fff;background:#2d6a4f;">Kgs. Aceituna</th>
      <th style="text-align:right;padding:5px;width:18%;color:#fff;background:#2d6a4f;">Kgs. Aceite</th>
      <th style="text-align:right;padding:5px;width:16%;color:#fff;background:#2d6a4f;">&euro;/Kg / Ratio</th>
      <th style="text-align:right;padding:5px;width:18%;color:#fff;background:#2d6a4f;">Valor (&euro;)</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      ...
      <td style="text-align:right;">' . number_format($prodEstimadaAceite, 2, ',', '.') . ' Kg</td>
      <td style="text-align:right;">4,18 &euro;/Kg <sup>1</sup></td>
      <td style="text-align:right;">' . m2_eur($prodEstimadaAceite * $precioAceite) . '</td>
    </tr>
    <tr>
      <td><strong>Campa&ntilde;a 2025/2026 (Real recolectada)</strong></td>
      <td style="text-align:right;">' . number_format($prodRealKg, 0, ',', '.') . ' Kg</td>
      <td style="text-align:right;">' . number_format($prodRealAceite, 2, ',', '.') . ' Kg</td>
      <td style="text-align:right;">4,18 &euro;/Kg</td>
      <td style="text-align:right;">' . m2_eur($prodRealAceite * $precioAceite) . '</td>
    </tr>
    <tr style="background:#f9fbf9;font-weight:bold;">
      <td>Subtotal 1. P&eacute;rdida producci&oacute;n directa (25/26)</td>
      <td style="text-align:right;">' . number_format(max(0, $prodEstimadaKg - $prodRealKg), 0, ',', '.') . ' Kg</td>
      <td style="text-align:right;">' . number_format($perdidaProdKgAceite, 2, ',', '.') . ' Kg</td>
      <td style="text-align:right;">4,18 &euro;/Kg</td>
      <td style="text-align:right;color:#1b4332;">' . m2_eur($valorPerdidaProd) . '</td>
    </tr>
    <tr>
      <td><strong>Subtotal 2. P&eacute;rdida pr&oacute;xima cosecha (26/27)</strong> (' . ($reduccionFisiologica * 100) . '% asfixia)</td>
      <td style="text-align:right;">' . number_format($baseProxKg * $reduccionFisiologica, 0, ',', '.') . ' Kg</td>
      <td style="text-align:right;">' . number_format($baseProxAceite * $reduccionFisiologica, 2, ',', '.') . ' Kg</td>
      <td style="text-align:right;">4,18 &euro;/Kg</td>
      <td style="text-align:right;font-weight:bold;color:#1b4332;">' . m2_eur($valorPerdidaProxima) . '</td>
    </tr>
    <tr>
      <td><strong>Subtotal 3. Depreciaci&oacute;n de calidad</strong> (Aceituna de suelo entregada)</td>
      <td style="text-align:right;">' . number_format($kgSueloAceituna, 0, ',', '.') . ' Kg</td>
      <td style="text-align:right;">' . number_format($kgSueloAceite, 2, ',', '.') . ' Kg</td>
      <td style="text-align:right;">1,50 &euro;/Kg <sup>2</sup></td>
      <td style="text-align:right;font-weight:bold;color:#1b4332;">' . m2_eur($valorDepreciacion) . '</td>
    </tr>
    <tr>
      <td><strong>Subtotal 4. Sobrecostes recolecci&oacute;n y almazara</strong></td>
      <td style="text-align:right;">' . number_format($kgSueloAceituna, 0, ',', '.') . ' Kg</td>
      <td style="text-align:right;">&mdash;</td>
      <td style="text-align:right;">' . number_format($costeRecoleccionRef * $pctSobrecoste + $costeAlmazara, 2, ',', '.') . ' &euro;/Kg ac. <sup>3</sup></td>
      <td style="text-align:right;font-weight:bold;color:#1b4332;">' . m2_eur($valorSobrecostes) . '</td>
    </tr>
    <tr>
      ...
      <td style="text-align:right;font-weight:bold;color:#1b4332;">' . m2_eur($valorSobrecostesExtra) . '</td>
    </tr>
  </tbody>
  <tfoot>
...
<h3>Titular</h3>
<table class="datos">
  <tr><th>Nombre / Raz&oacute;n Social</th><td>' . m2_h($razonSocial) . '</td></tr>
  <tr><th>DNI / CIF / NIE</th><td>' . m2_h($cifNif) . '</td></tr>
</table>
' . (($repNombre || $repDni) ? '
<h3>Representante</h3>
<table class="datos">
  ' . ($repNombre ? '<tr><th>Nombre y apellidos</th><td>' . m2_h($repNombre) . '</td></tr>' : '') . '
  ' . ($repDni ? '<tr><th>DNI/NIE</th><td>' . m2_h($repDni) . '</td></tr>' : '') . '
</table>' : '') . '
<h3>Domicilio</h3>
<table class="datos">
...
<div class="firma-bloque">
  ...
  ' . $firmaImgTag . '
  <p>' . m2_h($repNombre ?: $razonSocial) . '<br>' . m2_h($repDni ?: $cifNif) . '</p>
</div>
...
if (!empty($_FILES['adjuntos']['name'][0])) {
    ...
    if ($totalAdj > MAX_ADJUNTOS) {
        m2_redirect('Se permiten como máximo ' . MAX_ADJUNTOS . ' documentos adjuntos.');
    }
    for ($i = 0; $i < count($archivosAdj['name']); $i++) {
        ...
        if ($archivosAdj['size'][$i] > MAX_TAMANO_ADJUNTO) {
            m2_redirect('Uno o más adjuntos superan el tamaño máximo de 8 MB.');
        }
        $finfo    = new finfo(FILEINFO_MIME_TYPE);
        $mimeReal = $finfo->file($archivosAdj['tmp_name'][$i]);
        if (!array_key_exists($mimeReal, TIPOS_ADJUNTO)) {
            m2_redirect('Tipo de archivo no permitido en adjuntos. Solo PDF e imágenes (JPG, PNG, WebP).');
        }
